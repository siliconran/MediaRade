...
const live = [];   // every child we started, so we can reap them on unload

function untrack(child) {
  const i = live.indexOf(child);
...
function run(exe, args, opts) {
  opts = opts || {};
  const cp = CEP.cp;
  let child = null, killed = false, timer = null;
  ...
  const promise = new Promise(function (resolve, reject) {
    ...
    function pump(chunk, isErr) {
      const buf = (isErr ? errBuf : outBuf) + chunk;
      const lines = buf.split(/\r?\n/);
      if (isErr) errBuf = lines.pop(); else outBuf = lines.pop();   // keep the partial tail
...
export const Proc = { run: run, runOk: runOk, which: which, killAll: killAll, live: live };
export default Proc;
