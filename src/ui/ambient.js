...
let canvas, ctx, cubes = [], raf = null, w = 0, h = 0, last = 0;
...
function seedCubes() {
  cubes = [];
  const count = Math.round(U.clamp((w * h) / 26000, 8, 34));
  for (let i = 0; i < count; i++) cubes.push(spawn(true));
}
...
function draw(now) {
  ...
  for (let i = 0; i < cubes.length; i++) {
    ...
    const depth = U.clamp(1 - (c.y / h) * 0.55, 0.25, 1);
